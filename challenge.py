...
while (True):
    ...
    our_rank = bot_names.index('cbteamname')
    print("[INFO] Your bot is ranked " + str(our_rank+1))

    random.seed(os.urandom(8))
    opponent_queue = []
    #three bots with lower rank
    opponent_queue += ([bot_names[our_rank-1],
			bot_names[our_rank-2],
			bot_names[our_rank-3]])
